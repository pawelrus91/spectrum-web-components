...
Spectrum Workflow Icons delivered in a mutable template tag so that they can be leveraged across various frameworks. The default export of this package pre-applies the `html` template tag from `lit-html` for ease of use in the Spectrum Web Components library. Continue reading below for information on using these icons in projects powered by other frameworks.
...
import { Circle } from '@spectrum-web-components/icons-workflow';
...
```js
import { Circle } from '@spectrum-web-components/icons-workflow/lib/icons.js';
